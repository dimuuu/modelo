...
describe("WebMCP argument schemas", () => {
  ...
  it("refuses a percent slider written as a ratio, and says how to write it", () => {
    ...
    const refused = slider(1);
    expect(refused.success).toBe(false);
    expect(JSON.stringify(refused.error?.issues)).toContain("21 means 21 %");
    expect(slider(100).success).toBe(true);
  });
