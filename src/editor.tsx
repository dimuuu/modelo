    ...
    <div className="config-row">
      ...
      {block.props.format === "currency" && (
        <label>
          ...
          >
            {unknownCurrency && (
              <option value={block.props.currency}>
                {block.props.currency}
              </option>
            )}
      ...
      {block.props.format === "unit" && (
        <label>
          ...
          >
            {unknownUnit && (
              <option value={block.props.unit}>{block.props.unit}</option>
            )}
          ...
          onChange={(event) => {
            ...
            set({
              decimals:
                raw === ""
                  ? -1
                  : Math.max(0, Math.min(8, Math.round(Number(raw)))),
            });
          }}
        />
      </label>
...
function SliderFields({ block, editor }: any) {
  ...
  return (
    <div className="config-row slider-config">
      <label>
        Min
        <input
          aria-label="Slider minimum"
          type="number"
          value={block.props.min}
          onChange={(event) => setBound("min", event.currentTarget.value)}
        />
      </label>
      <label>
        Max
        <input
          ...
          onChange={(event) => setBound("max", event.currentTarget.value)}
        />
      </label>
      <label>
        Step
        <input
          ...
          type="number"
          value={block.props.step}
          onChange={(event) => setBound("step", event.currentTarget.value)}
        />
      </label>
    </div>
  );
...
function SelectOptions({ block, editor }: any) {
  ...
  return (
    <div className="select-options" aria-label="Select options">
    ...
    render: ({ block, editor }) => (
      <div className="model-block">
        ...
        <div className="number-row">
          ...
          />
          <Value varId={block.props.varId} fallback={block.props.value} />
        </div>
...
const BooleanBlock = createReactBlockSpec(
  ...
  {
    render: ({ block, editor }) => (
      <div className="model-block boolean-block">
        <div className="model-meta">
          <strong>{block.props.label}</strong>
          <VariableName block={block} editor={editor} />
        </div>
        <div className="boolean-row">
          <button
            type="button"
            role="switch"
            aria-checked={Boolean(block.props.value)}
            aria-label={block.props.label}
            className={`toggle ${block.props.value ? "on" : ""}`}
            onClick={() =>
              updateProps(editor, block, { value: block.props.value ? 0 : 1 })
            }
          >
            <span />
          </button>
...
const FormulaBlock = createReactBlockSpec(
  ...
  {
    render: ({ block, editor }) => (
      <div className="model-block formula-block">
        <div className="model-meta">
          <strong>{block.props.label}</strong>
          <VariableName block={block} editor={editor} />
        </div>
...
const VariableRef = createReactInlineContentSpec(
  ...
  },
  {
    render: ({ inlineContent }) => {
      ...
      return (
        <span
          className={`variable-chip ${variable?.status ?? "missing"}`}
          title={
