/**
 * The tab strip: which notebooks are open, and which tab is in front.
 *
 * A tab is one notebook id, or null for home. The list and the active index
 * are the whole state, and they live in the query string
 * (`src/use-tab-state.ts`), so a reload or a shared link opens the same tabs.
 *
 * A notebook is open in at most one tab, so its editor and its saved blocks
 * never fork. Every change is a pure function from one `TabState` to the next,
 * the same shape the workspace catalogue uses.
 */
...
export interface TabState {
  /** One entry per tab: the notebook it shows, or null for home. */
  tabs: (string | null)[];
  /** The index of the tab in front. Always inside `tabs`. */
  active: number;
}
...
export function pruneTabs(state: TabState, existing: Set<string>): TabState {
  ...
  let next = state;
  // Backwards, so closing one tab cannot shift the index of the next to close.
