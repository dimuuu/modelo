...
math.createUnit("percent", { aliases: ["pct"], definition: "0.01" });
...
function formatKind(variable: ProjectedInput): string | undefined {
  return typeof variable.format === "string"
    ? variable.format
    : variable.format?.style;
}
...
function currencyCode(
  ...
  const nested =
    typeof variable.format === "object" && variable.format.style === "currency"
      ? variable.format.currency
      : undefined;
  return (
    variable.currency ||
    nested ||
...
function ensureCurrency(code: string): void {
  if (registeredCurrencies.has(code)) {
    return;
  }
...
function inputQuantity(
  ...
  if (kind === "unit" && variable.unit) {
    return math.unit(variable.value, variable.unit);
  }
    ...
    if (variable.kind === "input") {
      if (Number.isFinite(variable.value)) {
        try {
          ...
          result = { ...variable, formatted, status: "ok" };
        } catch (error) {
          result = errorResult(
            variable,
        ...
        result = errorResult(variable, "Input must be a finite number");
      }
    } else {
