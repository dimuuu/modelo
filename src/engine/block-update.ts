...
function checkNumericRules(
  ...
  const percent = percentRangeProblem({ format, max, min });
  if (percent) {
    return problem("INVALID_VALUE", percent);
  }
  return undefined;
}
...
/**
 * Decides what one `update_block` call does to one block, without touching the
 * editor. `model` is the current projection, used for name uniqueness.
 */
