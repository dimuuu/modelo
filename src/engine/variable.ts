...
/**
 * The fields a stored input block may carry. Currency and unit are free strings
 * here, because a notebook may hold a code the picker does not offer; the
 * agent-facing schemas narrow them to the curated lists.
 */
/**
 * A percent input stores whole percents: 21 means 21 %. A slider whose whole
 * range fits under 1 % is a ratio written by mistake, and it is refused with a
 * message that says how to write it.
 */
export function percentRangeProblem(fields: {
  format?: unknown;
  min?: unknown;
  max?: unknown;
}): string | undefined {
  if (fields.format !== "percent" || typeof fields.max !== "number") {
    return undefined;
  }
  const min = typeof fields.min === "number" ? fields.min : 0;
  if (min >= 0 && fields.max <= 1) {
    return "Percent values are whole percents: 21 means 21 %. Give a percent slider a range such as min 0, max 100, not 0 to 1.";
  }
  return undefined;
}
...
/**
 * The value an input block stores for a requested value. Booleans store 0 or 1;
 * sliders stay inside their bounds; everything else keeps its number.
 */
