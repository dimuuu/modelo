 ...
 *
 * Seven operations are everything the tools and the block components need.
 * Two adapters implement them: BlockNote in the app, and the in-memory
 * adapter below for tests and for `dry_run` previews. Two adapters make the
 * seam real.
 */
...
export interface EditorPort {
  ...
  /** Inserts next to `referenceId`, at any depth. New blocks without an id get one. */
  insertBlocks: (
  ...
  /** Groups mutations into one undo step. */
  transact: <T>(fn: () => T) => T;
}
...
/** Splices `blocks` next to the block with `referenceId`, wherever it is. */
function insertAt(
