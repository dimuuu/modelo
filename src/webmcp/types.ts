...
export interface NotebookGetModelArgs {
...
export interface NotebookWriteSectionArgs {
  ...
  formulas?: { name: string; formula: string; label?: string }[];
  referenceBlockId?: string;
  placement?: "before" | "after";
  dry_run?: boolean;
}
export interface NotebookWriteSectionsArgs {
  sections: NotebookWriteSectionArgs[];
  dry_run?: boolean;
}

interface InputUpdateFields {
  name?: string;
  label?: string;
  value?: number;
  format?: "number" | "currency" | "percent" | "unit";
...
export interface NotebookUpdateBlocksArgs {
  blocks: NotebookUpdateBlockArgs[];
}

export interface NotebookRemoveBlocksArgs {
  ids: string[];
}
export type NotebookVariableSelector =
  | { name: string; varId?: never }
  | { name?: never; varId: string };
export type NotebookFindReferencesArgs = NotebookVariableSelector;
export type NotebookRemoveVariableArgs = NotebookVariableSelector & {
  force?: boolean;
};
...
export interface NotebookSaveScenarioArgs {
  name: string;
  values?: Record<string, number>;
}
export interface NotebookScenarioArgs {
  name: string;
}
