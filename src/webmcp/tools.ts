...
function noNotebookOpen(): never {
  return fault("NO_NOTEBOOK_OPEN", "Open a notebook first.");
}
...
export class ToolContext {
  ...
  session(): NotebookSession {
    const editor = this.runtime.editor() ?? noNotebookOpen();
...
function scenarioSummary({ id, name }: Scenario) {
  return { id, name };
}

function recordView(record: NotebookRecord) {
  return { ...record, title: notebookTitle(record) };
}
...
function unresolvedWarnings(
  ...
  return unresolvedReferences(text, idByName).map(
...
function prepareSection(
  ...
  return {
    editorBlocks: toEditorBlocks(portable, idByName),
    warnings: unresolvedWarnings(section.body, idByName),
  };
}
...
const notebookTools: ToolDefinition[] = [
  ...
  tool({
    ...
    run: (context, { blocks, referenceBlockId, placement }) => {
      ...
      const warnings = blocks.flatMap((block) =>
        "text" in block ? unresolvedWarnings(block.text, idByName) : []
      );
  ...
  tool({
    description:
      "Add one prose-first section: a heading, body text that mentions variables as @Name, and the inputs and formulas behind them. Percent inputs store whole percents (21 means 21 %) and reach formulas as ratios (0.21), so a percent slider runs 0 to 100. Use format number for counts and loan terms; unit year means a duration. Supports dry_run.",
    name: "write_section",
  ...
  tool({
    description:
      "Add several sections atomically, or preview with dry_run. Percent inputs store whole percents (21 means 21 %) and reach formulas as ratios, so Price * (1 + TaxRate) is right and a percent slider runs 0 to 100. Formula examples: 1 + MortgageRate; Principal * Rate / 12; 5 km + 500 m. Use format number for counts and loan terms; unit year is a duration.",
    name: "write_sections",
...
/**
 * Runs one tool by name against a runtime: checks the notebook is open when the
 * tool needs one, validates the arguments, executes, and wraps the result. This
 * is the interface the WebMCP hook registers and the tests call.
 */
