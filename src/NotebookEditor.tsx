...
import { focusVariableName } from "./editor-dom";
...
const MODEL_BLOCKS: {
...
}[] = [
  {
    ...
    kind: "number",
    subtext: "Named number input",
    title: "Number",
  },
  {
    icon: <SlidersHorizontalIcon size={18} />,
    kind: "slider",
    subtext: "Named slider input",
    title: "Slider",
  },
  {
    icon: <ListIcon size={18} />,
    kind: "select",
    subtext: "Named select input",
    title: "Select",
  },
  {
    icon: <ToggleLeftIcon size={18} />,
    kind: "boolean",
    subtext: "Named boolean input",
    title: "Toggle",
  },
  {
    icon: <SigmaIcon size={18} />,
    kind: "formula",
    subtext: "Computed MathJS expression",
    title: "Formula",
  },
];
...
export function NotebookEditor({
  ...
  const slashItems = useCallback(
    (query: string) => {
      const modelItems = MODEL_BLOCKS.map(({ kind, title, subtext, icon }) => ({
        group: "Modelo",
        icon,
        onItemClick: () => {
          ...
          focusVariableName(block.id);
        },
        subtext,
        title,
      }));
      const all = [...modelItems, ...getDefaultReactSlashMenuItems(editor)];
