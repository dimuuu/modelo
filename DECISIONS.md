...
- **A model block is one line.** Name, one control, one value, and nothing else. Every setting moved into the six-dot menu, where format, currency, unit, and decimals are submenus. The rejected alternative was the configuration row under each block: it doubled the height of every input, and a page of ten inputs read as a form rather than as a document.
...
- **The font ships with the app.** Inter comes from the official release at `github.com/rsms/inter` as two variable files in `src/fonts`, not from a font CDN. Vite hashes and emits them, so there is no third-party request and no flash of a fallback face from a slow CDN. The two files carry every weight and the optical size axis, which is what Inter Display is; the notebook title pins that axis to 32 rather than loading a second family.
